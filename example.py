...
Example usage of the modularized DeepAgent system.
"""

import os
from deep_agent import create_deep_agent, SubAgentConfig
from dotenv import load_dotenv
...
research_agent = SubAgentConfig(
    ...
    prompt="You are a research specialist. Focus on gathering comprehensive information.",
    tools=["custom_search", "write_file", "read_file"]
)

analysis_agent = SubAgentConfig(
    name="analysis-agent", 
    description="Specialized in data analysis and processing",
    prompt="You are an analysis expert. Focus on processing and analyzing data.",
...
agent = create_deep_agent(
    tools=[custom_search],
    instructions="You are a comprehensive research and analysis agent.",
    subagents=[research_agent, analysis_agent, report_agent],
    api_key=os.getenv("ANTHROPIC_API_KEY"),
    model="claude-sonnet-4-20250514"
)
...
if __name__ == "__main__":
    input_data = {
        "messages": [
            {
                "role": "user", 
                "content": "Research and analyze the current state of renewable energy technology, then create a comprehensive report."
            }
        ],
        "files": {}
    }
