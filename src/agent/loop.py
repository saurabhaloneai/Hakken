...
class StreamingResponseHandler:
    def __init__(self, agent):
        self.agent = agent
        self.ui = agent.ui
        self.api_client = agent.api_client
        ...
        try:
            if hasattr(stream, '__aiter__'):
                async for chunk in stream:
                    if self._process_chunk(chunk, content_parts):
                        interrupted = True
                        break
                    message, usage = self._extract_message_data(chunk, message, usage)
            else:
                for chunk in stream:
    ...
    def _process_chunk(self, chunk, content_parts: List[str]) -> bool:
        if self.agent._check_interrupt():
            return True
    ...
    def _extract_message_data(self, chunk, message, usage):
...
class ToolExecutor:
    def __init__(self, agent):
        ...
        self.config = agent.config
        self.interrupts = agent.interrupts
    ...
    async def execute_tools(self, message: Any) -> None:
        ...
        tools = await self._parse_tool_calls(message.tool_calls)
        approved_tools = await self._get_approvals(tools)
        
        await self._execute_approved_tools(approved_tools, tools, message.tool_calls, pending)

    async def _parse_tool_calls(self, tool_calls) -> List[Dict]:
        tools = []
    ...
    async def _execute_parallel_tools(self, parallel_tools: List[Dict], pending: str, 
        ...
        for tool, result in zip(parallel_tools, results):
            ...
            is_last = tool['index'] == total_tools - 1 and not sequential_tools
...
class ConversationFlow:
    ...
    async def process_cycle(self) -> None:
        ...
        if response.interrupted:
            ...
            await self.agent.tool_executor.execute_tools(response.message)
            await self.process_cycle()
    ...
    async def _handle_nudges(self) -> bool:
        try:
            ...
            # Handle specific nudge patterns
            if await self._check_todo_nudge(last_content):
                return True
            if await self._check_directory_nudge(last_content):
                return True
            if await self._check_file_nudge(last_content):
                return True
    ...
    async def _check_todo_nudge(self, content: str) -> bool:
        if ("check todo.md" in content or "check the todo.md" in content) and \
           ("let me" in content or "i'll" in content or "i will" in content):
            await self.agent._add_message("user", "use read_file to open 'todo.md' now")
            return True
        return False

    async def _check_directory_nudge(self, content: str) -> bool:
        if "let me check" in content and "directory" in content and \
           ("ls" in content or "list" in content):
            await self.agent._add_message("user", "use cmd_runner with 'ls -la' now")
            return True
        return False

    async def _check_file_nudge(self, content: str) -> bool:
        if ("let me open" in content or "i'll open" in content) and "file" in content:
            await self.agent._add_message("user", "use read_file to open the stated file now")
            return True
        return False

    async def _handle_pending(self) -> bool:
        """Handle any pending instructions"""
        pending = self.agent.state.get('pending_instruction', '').strip()
        if pending:
            self.agent.state['pending_instruction'] = ''
            await self.agent._add_message("user", pending)
            return True
        return False
...
class ConversationAgent:    
    ...
    async def start_task(self, system_prompt: str, user_input: str) -> str:
        ...
        try:
            await self.conversation_flow.process_cycle()
